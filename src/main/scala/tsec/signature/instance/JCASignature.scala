...
class JCASignature[F[_], A](implicit M: Async[F], signatureAlgorithm: SignatureAlgorithm[A]) extends SignatureAlgebra[F, A]{
  type S = Signature
  type PubK = PublicKey
  ...
  def initVerifyC(c: SigCertificate[@@[Certificate, A]], instance: Signature): F[Unit] = M.delay(instance.initVerify(c.certificate))
