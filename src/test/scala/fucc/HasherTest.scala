...
class HasherTest extends TestSpec {
  ...
  "Batch hashing and sequential hashing" should "be the same for SHA1" in {
    val batchList = SHA1.hashBatch.apply(strList)
  ...
  it should "be the same for MD5" in {
    val batchList = MD5.hashBatch.apply(strList)
  ...
  it should "be the same for SHA256" in {
    val batchList = SHA256.hashBatch.apply(strList)
  ...
  it should "be the same for SHA512" in {
    val batchList = SHA512.hashBatch.apply(strList)
  ...
  "Batch hashing" should "return same size" in {
    ...
    assert(
      list0.length == SHA1.hashBatch.apply(list0).length &&
        list1.length == SHA1.hashBatch.apply(list1).length &&
        list2.length == SHA1.hashBatch.apply(list2).length &&
        list2.length == SHA1.hashBatch.apply(list2).length
    )
  }
