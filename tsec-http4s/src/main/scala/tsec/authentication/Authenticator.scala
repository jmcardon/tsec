...
sealed abstract class Authenticator[F[_], Alg, Identity, User, Auth[_]](
    val authenticator: AuthenticatorEV[F, Alg, Identity, User, Auth]
  ...
  protected def authorizedMiddleware(authorization: Authorization[User]): TSecMiddleware[F, Auth[Alg], User] = {
  ...
  def authorized(authorization: Authorization[User])(
...
object Authenticator {
  ...
  )(implicit F: MonadError[F, Throwable]): Authenticator[F, Alg, Identity, User, Auth] = {
    val middleware = TSecMiddleware(Kleisli(authenticator.extractAndValidate))
    if (rolling) {
      new Authenticator[F, Alg, Identity, User, Auth](authenticator) {
        def apply(pf: PartialFunction[SecuredRequest[F, Auth[Alg], User], F[Response[F]]]): HttpService[F] =
  ...
  private[authentication] final class DefaultEncrypted[F[_]](val dummy: Boolean = true) extends AnyVal {
    def apply[Alg, Identity, User](
      ...
      Authenticator[F, Alg, Identity, User, AuthEncryptedCookie[?, Identity]](authenticator, rolling)
  }
  ...
  private[authentication] final class DefaultCookie[F[_]](val dummy: Boolean = true) extends AnyVal {
    def apply[Alg, Identity, User](
      ...
      Authenticator[F, Alg, Identity, User, AuthenticatedCookie[?, Identity]](authenticator, rolling)
  }
  ...
  private[authentication] final class DefaultJWT[F[_]](val dummy: Boolean = true) extends AnyVal {
    def apply[Alg, Identity, User](
      ...
      Authenticator[F, Alg, Identity, User, JWTMac](authenticator, rolling)
  }
