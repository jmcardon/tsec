...
class JCAAEAD[F[_], A, M, P](queue: JQueue[JCipher])(
  ...
  def genInstance: F[JCipher] = F.delay {
    ...
    if (inst != null)
      ...
      JCAAEAD.getJCipherUnsafe[A, M, P]
  }
...
object JCAAEAD {
  ...
  def apply[F[_], A: AEADCipher, M: AEADMode, P: Padding](
      queueLen: Int = 15
  )(implicit F: Sync[F]): F[JCAAEAD[F, A, M, P]] =
    for {
      q <- F.delay(genQueueUnsafe(queueLen))
    } yield new JCAAEAD[F, A, M, P](q) {}

  implicit def genInstance[F[_]: Sync, A: AEADCipher, M: AEADMode, P: Padding]: F[JCAAEAD[F, A, M, P]] =
    apply[F, A, M, P]()
