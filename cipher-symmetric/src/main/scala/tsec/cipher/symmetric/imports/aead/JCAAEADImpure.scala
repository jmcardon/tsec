...
object JCAAEADImpure {
  ...
  /** Attempt to initialize an instance of the cipher with the given type parameters
    ...
    * @return
    */
  def apply[A: AEADCipher, M: AEADMode, P: Padding]: Either[NoSuchInstanceError.type, JCAAEADImpure[A, M, P]] = {
    val c = new JCAAEADImpure[A, M, P]
    c.genInstance.map(_ => c).leftMap(_ => NoSuchInstanceError)
  }

  implicit def genSym[A: AEADCipher, M: AEADMode, P: Padding]: Either[NoSuchInstanceError.type, JCAAEADImpure[A, M, P]] =
    apply[A, M, P]
  ...
  /** ┌(▀Ĺ̯▀)–︻╦╤─ "You will never get away with an unsafe instance!!"
    ...
    * @return
    */
  def getCipherUnsafe[A: AEADCipher, M: AEADMode, P: Padding]: JCAAEADImpure[A, M, P] =
    new JCAAEADImpure[A, M, P]
