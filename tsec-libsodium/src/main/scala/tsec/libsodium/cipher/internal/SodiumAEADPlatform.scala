...
trait SodiumAEADPlatform[A]
  ...
  final def decrypt[F[_]](cipherText: CipherText[A], key: SodiumKey[A])(
      implicit F: Sync[F],
      S: ScalaSodium
  ): F[PlainText] = F.delay {
  ...
  final def encryptAAD[F[_]](plaintext: PlainText, key: SodiumKey[A], aad: AAD)(
      implicit F: Sync[F],
      S: ScalaSodium
  ): F[CipherText[A]] = F.delay {
    val outArray = RawCipherText[A](new Array[Byte](plaintext.length + authTagLen))
  ...
  final def decryptAAD[F[_]](cipherText: CipherText[A], key: SodiumKey[A], aad: AAD)(
      implicit F: Sync[F],
      S: ScalaSodium
  ): F[PlainText] = F.delay {
    val originalMessage = new Array[Byte](cipherText.content.length - authTagLen)
  ...
  final def encryptAADDetached[F[_]](plainText: PlainText, key: SodiumKey[A], aad: AAD)(
      implicit F: Sync[F],
      S: ScalaSodium
  ): F[(CipherText[A], AuthTag[A])] = F.delay {
  ...
  final def decryptAADDetached[F[_]](cipherText: CipherText[A], key: SodiumKey[A], authTag: AuthTag[A], aad: AAD)(
      implicit F: Sync[F],
      S: ScalaSodium
  ): F[PlainText] = F.delay {
