...
class ValidatedRequestSpec extends FlatSpec {
  ...
  it should "fetch authorization header without colon" in {
    val parsedCred =
      ValidatedRequest.parseClientCredential(Map("Authorization" -> Seq("Basic Y2xpZW50X2lkX3ZhbHVl")), Map.empty)
    parsedCred.isLeft shouldBe true
  }

  it should "fetch empty client_secret with colon" in {
    val c = ValidatedRequest
      .parseClientCredential(Map("Authorization" -> Seq("Basic Y2xpZW50X2lkX3ZhbHVlOg==")), Map.empty)
      .toOption
      .get
    c.clientId should be("client_id_value")
  ...
  it should "not fetch not Authorization key in header" in {
    val c = ValidatedRequest.parseClientCredential(
      Map("authorizatio" -> Seq("Basic Y2xpZW50X2lkX3ZhbHVlOmNsaWVudF9zZWNyZXRfdmFsdWU=")),
      Map.empty
    )
    c should be(
      Left(InvalidClient(s"Failed to parse client credential from header (Missing authorization header) and params"))
    )
  }
  ...
  it should "not fetch invalid Base64" in {
    ...
    parsedCred shouldBe Left(
      InvalidClient("Failed to parse client credential from header (invalid Base 64) and params")
    )
  }
  ...
  it should "not fetch missing parameter" in {
    ...
    c should be(
      Left(InvalidClient("Failed to parse client credential from header (Missing authorization header) and params"))
    )
  }
  ...
  it should "not fetch invalid Authorization header" in {
    val parsedCred =
      ValidatedRequest.parseClientCredential(Map("Authorization" -> Seq("Digest Y2xpZW50X2lkX3ZhbHVlOg==")), Map.empty)

    parsedCred shouldBe Left(InvalidAuthorizationHeader)
  }

  it should "not fetch if Authorization header is invalid, but client_id and client_secret are valid and present in parms" in {
    val parsedCred = ValidatedRequest.parseClientCredential(
